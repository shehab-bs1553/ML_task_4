...
def processing_image(image,filename,folder):
    ...
    save_into_file(gray_image,folder,filename,"_edges_image")
    #code for resize the images and save it
    resized_image = cv2.resize(image, (256,256))
    save_into_file(gray_image,folder,filename,"_resized_image")
    #code for blurring the images and save it
    blurred_image = cv2.GaussianBlur(image, (5, 5), 0)
    save_into_file(gray_image,folder,filename,"_blured_image")
    ...
    save_into_file(gray_image,folder,filename,"_histogram_image")
    #add global and adaptive threshold
    _, global_threshold = cv2.threshold(gray_image, 127, 255, cv2.THRESH_BINARY)
    save_into_file(gray_image,folder,filename,"_global_threshold_image")              
